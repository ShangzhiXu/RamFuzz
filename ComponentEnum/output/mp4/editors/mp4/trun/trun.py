...
def trun(fp, values, fields, debug=False):
    """Edit trun box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] trun box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] trun.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] trun.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] trun.{field} after: {value}")
