...
def dtsl(fp, values, fields, debug=False):
    """Edit dtsl box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] dtsl box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] dtsl.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] dtsl.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] dtsl.{field} after: {value}")
