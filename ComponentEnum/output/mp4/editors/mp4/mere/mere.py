...
def mere(fp, values, fields, debug=False):
    """Edit mere box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] mere box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] mere.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] mere.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] mere.{field} after: {value}")
