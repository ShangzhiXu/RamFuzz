...
def alac(fp, values, fields, debug=False):
    """Edit alac box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] alac box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] alac.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] alac.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] alac.{field} after: {value}")
