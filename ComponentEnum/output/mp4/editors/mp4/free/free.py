...
def free(fp, values, fields, debug=False):
    """Edit free box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] free box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] free.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] free.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] free.{field} after: {value}")
