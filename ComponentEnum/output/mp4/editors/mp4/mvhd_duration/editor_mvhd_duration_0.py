...
def mp4_mvhd_duration(fp, value, debug=False):
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            # The 'mvhd' box starts with a 4-byte size and 'mvhd' type, followed by version and flags (4 bytes)
            # The duration is located at a fixed offset from the start of the 'mvhd' box
            # Offset: 16 bytes for version and flags + 4 bytes for creation time + 4 bytes for modification time
            duration_offset = mvhd_pos + 16 + 4 + 4
