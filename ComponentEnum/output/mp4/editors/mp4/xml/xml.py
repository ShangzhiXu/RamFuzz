...
def xml(fp, values, fields, debug=False):
    """Edit xml box fields in an MP4 file.
        ...
        with open(fp, 'r+b') as f:
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] xml.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] xml.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] xml.{field} after: {value}")
