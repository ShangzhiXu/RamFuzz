...
def ttml(fp, values, fields, debug=False):
    """Edit ttml box fields in an MP4 file.
    ...
    try:
        with open(fp, 'r+b') as f:
            ...
            if pos == -1:
                if debug:
                    print("[DEBUG] ttml box not found")
                return
            box_start = pos - 4
            ...
            for i, field in enumerate(fields):
                if field not in FIELD_MAP:
                    if debug:
                        print(f"[DEBUG] ttml.{field}: unknown field, skipping")
                    continue
                ...
                if debug:
                    print(f"[DEBUG] ttml.{field} before: {old_val}")
                ...
                if debug:
                    print(f"[DEBUG] ttml.{field} after: {value}")
