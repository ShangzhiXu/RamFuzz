...
FormatFuzzer includes **enhanced templates** with comprehensive field coverage for 13 formats, plus original templates for additional formats.

### Enhanced Templates (with full component/type-value JSON specs)

| Format | Template | JSON Fields | Description |
|--------|----------|-------------|-------------|
| MP4    | `mp4_enhanced.bt` | 04_type_values.json | ISO BMFF, fragmented MP4, HEIF, DRM/CENC, Dolby Vision, AV1, HEVC, Opus, FLAC, AC-3/4, DTS |
| MP3    | `mp3_enhanced.bt` | 239 fields | Frame header, ID3v1/v2 (73+ frame types), Xing/LAME/VBRI, APE, Lyrics3v2 |
| JPEG   | `jpeg_enhanced.bt` | 239 fields | JFIF, Exif (IFD0/ExifIFD/GPS/Interop), ICC, IPTC/Photoshop, Adobe APP14, XMP |
| PNG    | `png_enhanced.bt` | 187 fields | 29 chunk types incl. APNG (acTL/fcTL/fdAT), cICP, oFFs, pCAL, sCAL, sTER, dSIG |
| GIF    | `gif_enhanced.bt` | 98 fields | GIF89a, GCE, Comment/PlainText ext, NETSCAPE/XMP/ICC/ANIMEXTS app ext |
| BMP    | `bmp_enhanced.bt` | 91 fields | BITMAPINFOHEADER/V4/V5, color masks, ICC profile |
| AVI    | `avi_enhanced.bt` | 132 fields | avih, strh, strf (video+audio), idx1, INFO list, vprp, indx, odml |
| WAV    | `wav_enhanced.bt` | 141 fields | fmt, fact, smpl+loops, inst, cue+adtl, bext (BWF), iXML, acid, INFO |
| PDF    | `pdf_enhanced.bt` | 196 fields | Header, catalog, pages, font, content stream, xref, trailer, encryption |
| PCAP   | `pcap_enhanced.bt` | 101 fields | Global header, Ethernet, IPv4/IPv6, TCP/UDP, ICMP, ARP, DNS |
| ELF    | `elf_enhanced.bt` | 82 fields | ELF64 header, program/section headers, symbol table, dynamic, notes |
| FLV    | `flv_enhanced.bt` | 66 fields | Header, audio/video/script tags, AAC/AVC data, onMetaData |
| TIFF   | `tiff_enhanced.bt` | 36 fields | TIFF header, IFD entries (150+ tags), strips/tiles, Exif/GPS/GeoTIFF, ICC |

### Original Templates

AVI, BMP, GIF, JPG, MIDI, MP3, MP4, PCAP, PNG, WAV, ZIP, and more (see `templates/` for the full list).
...
│   ├── *_enhanced.bt       # Enhanced templates (13 formats)
│   ├── *.bt                # Original templates
│   └── *-orig.bt           # Unmodified original templates
├── build/                  # Compiled artifacts
│   ├── *_enhanced.cpp      # Generated C++ from enhanced templates
│   ├── *_enhanced.so       # Shared libraries (for AFL++ integration)
│   ├── *_enhanced-fuzzer   # Standalone fuzzer binaries
│   └── *_violation*.so     # Violation-aware variants
├── ComponentEnum/
│   └── output/
│       └── <format>/       # Per-format JSON specifications
│           ├── 01_initial_components.json
│           ├── 02_components_after_consistency.json
│           ├── 03_subcomponents.json
│           └── 04_type_values.json
├── pfp/                    # Python format parser framework
├── build.sh                # Build script (compiles .bt -> .so + variants)
├── ffcompile               # Binary template compiler (.bt -> .cpp)
├── fuzzer.cpp              # Generic fuzzer driver (with violation support)
├── fuzzer_orig.cpp         # Generic fuzzer driver (original)
